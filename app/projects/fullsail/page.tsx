...
export const metadata: Metadata = {
  title: "FullSail Case Study",
  description:
    "How FullSail was delivered as a production-ready Sui DEX across swap, liquidity, and governance workflows.",
};

export default async function FullSailCaseStudyPage() {
  const profile = await getGithubProfile(resumeData.githubUsername);
  const caseStudy = resumeData.projectCaseStudies?.fullsail;

  if (!caseStudy) {
    notFound();
  }

  const projectCard = resumeData.featuredProjects.find((project) => project.title === "FullSail");
      ...
      <main className="bg-[var(--surface)] text-[var(--text)]">
        <Section className="border-t-0 !pb-8 !pt-20 md:!pt-24">
          ...
          <EditorialHeading
            ...
            eyebrow="Case Study"
            title="FullSail"
            description="Sui-based Ve(3,3)-inspired DEX delivery from architecture through production rollout."
          />
        </Section>
        ...
        <CaseStudyLayout metadata={metadataItems}>
          <ScenarioSection
            title="Real Scenario"
            intro="What we had to solve in real launch conditions"
            ...
            <section className="space-y-4">
              ...
              <div className="overflow-hidden rounded-md border border-[var(--line)] bg-[var(--surface)]">
                <Image
                  ...
                  alt="FullSail landing page and product interface"
                  width={1600}
                  height={900}
          ...
          <OwnedChecklistSection
            title="Owned Scope"
            intro="Delivery responsibilities I directly owned"
